...
# !! THIS IS THE FINAL, CLEANED, AND CORRECTED FILE !!
from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session, joinedload
from typing import List
from app.api import deps
...
@router.post("/temples", response_model=schemas.Temple)
def create_temple(
    temple_in: schemas.TempleCreate,
    db: Session = Depends(deps.get_db),
    admin_user: models.User = Depends(deps.get_current_admin_user)
):
    return crud.temple.create(db, obj_in=temple_in)
...
@router.delete("/temples/{temple_id}", response_model=schemas.Temple)
def delete_temple(
    temple_id: int,
    db: Session = Depends(deps.get_db),
    admin_user: models.User = Depends(deps.get_current_admin_user)
...
@router.get("/promos", response_model=List[schemas.PromoCode])
def get_promo_codes(
    db: Session = Depends(deps.get_db),
    admin_user: models.User = Depends(deps.get_current_admin_user)
):
...
@router.delete("/promos/{promo_id}", response_model=schemas.PromoCode)
...
def get_temple_bookings(
    ...
    """
    Get all bookings for a specific temple.
    This version uses explicit JOINs to ensure data integrity and prevent
    errors from bookings linked to deleted temples or services.
    """
    bookings = (
        db.query(models.Booking)
        .join(models.Booking.temple)
        .join(models.Booking.pooja_service)
        .options(
            joinedload(models.Booking.stages)
        )
        .filter(models.Booking.temple_id == temple_id)
    ...
    # This transformation is safe because the query guarantees
    # that b.temple and b.pooja_service will not be None.
    response_bookings = []
    for b in bookings:
        response_booking = {
            "id": b.id,
...
@router.put("/bookings/{booking_id}/stages/{stage_id}", response_model=schemas.PoojaStage)
def update_booking_stage(
    booking_id: int,
    stage_id: int,
    update_data: schemas.BookingStageUpdate,
