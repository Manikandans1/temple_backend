...
# class CRUDTemple(CRUDBase[models.Temple, schemas.TempleCreate]): # <-- THIS LINE IS CORRECTED
#     def get_with_services(self, db: Session, *, id: int):
...
#             video_option=obj_in.video_option,
#             devotee_name=obj_in.devotee_name,
#             devotee_nakshatra=obj_in.devotee_nakshatra,
#             user_id=user_id,
#             temple_id=obj_in.temple_id,
#             pooja_service_id=obj_in.pooja_id
#         )
#         db.add(db_booking)
#         db.commit()
...
#                 name=member.name, nakshatra=member.nakshatra, booking_id=db_booking.id
#             )
#             db.add(db_member)
...
#     # def get_multi_by_owner(self, db: Session, *, user_id: int):
#     #     return db.query(self.model).filter(models.Booking.user_id == user_id).all()
    
#     def get_multi_by_owner(self, db: Session, *, user_id: int):
#         return db.query(self.model).filter(models.Booking.user_id == user_id).order_by(models.Booking.booking_date.desc()).all()
    
#         # NEW: Get bookings for a specific temple (for admins)
#     def get_multi_by_temple(self, db: Session, *, temple_id: int):
#         return db.query(self.model).filter(models.Booking.temple_id == temple_id).order_by(models.Booking.booking_date.desc()).all()
    
#     # NEW: Update a booking's stage
#     def update_stage(self, db: Session, *, booking_id: int, stage_id: int, status: models.StageStatus):
#         stage = db.query(models.PoojaStage).filter(models.PoojaStage.booking_id == booking_id, models.PoojaStage.id == stage_id).first()
#         if stage:
#             stage.status = status
...
# user = CRUDUser(models.User)
# temple = CRUDTemple(models.Temple)
# pooja_service = CRUDPoojaService(models.PoojaService) # <-- ADDED
# promo_code = CRUDPromoCode(models.PromoCode) # <-- ADDED
# booking = CRUDBooking(models.Booking)